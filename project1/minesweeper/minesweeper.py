    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # skip if (i, j) == cell or out of bounds
                if (i, j) == cell or not (0 <= i < self.height and 0 <= j < self.width):
                    continue

                # if (i, j) is not a known cell add it to the sentence
                if (i, j) in self.mines:
                    count -= 1
                elif (i, j) not in self.moves_made | self.safes:
                    neighbours.add((i, j))
            ...
            for s1 in self.knowledge.copy():
                for s2 in self.knowledge.copy():
                    ...
                    if s1.cells.issubset(s2.cells):
                        sentence = Sentence(s2.cells.difference(s1.cells), s2.count - s1.count)
                        ...
                        sentence.cells -= to_remove
                        sentence.count -= count_dec
